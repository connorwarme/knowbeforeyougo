  ...
  const topicLinks = topics.map((topic, i) => {
    ...
    return (
      <li key={i} className={uniqueStyle}>
        <Link
          to={`#${lowercase}`}
          state={{ index: i + chapterLinks.length + 1 }}
          onClick={handleNavClick}
          className="p-4"
        >
          {topic}
    ...
    <>
      <div className="z-10 relative px-8 md:px-12 py-4 bg-offwhite min-h-dvh min-w-[200px] md:min-w-[350px] animate-[translateX_350ms_ease-in-out_forwards]">
        <ul className="mt-[30%] *:my-4 h-auto transition-all duration-300 md:text-xl">
          <li>
            <Link to="/" className="" onClick={handleNavClick}>
              Home
            </Link>
          </li>
          <li className="relative">
            <div className="flex items-center justify-between">
              ...
              >
                <Expand
                  className={showResources ? expandStyleRotate : expandStyle}
                  title={showResources ? "Minimize" : "Expand"}
                  alt={`${showResources ? "Minimize" : "Expand"} the chapter links`}
                />
              </button>
            </div>
            <ul
              className={`top-full width-full left-0 z-0 font-semibold text-sm md:text-lg`}
            >
              {chapterLinks}
            </ul>
          </li>
          <li className="relative">
            <div className="flex items-center justify-between">
              ...
              >
                <Expand
                  className={showTopics ? expandStyleRotate : expandStyle}
                  title={showTopics ? "Minimize" : "Expand"}
                  alt={`${showTopics ? "Minimize" : "Expand"} the topic links`}
                />
              </button>
            </div>
            <ul
