    ...
    <>
      ...
      >
        <h3 className="font-display flex flex-col transition-all duration-300 lg:gap-2">
          ...
          <span className="self-center font-bold sm:text-2xl lg:text-4xl">
            {card.name}
          </span>
        </h3>
        <div
          className={selected === card.id ? containerStyleOpen : containerStyle}
        >
        ...
        <div className="button-container flex justify-center">
          <button>
            {selected === card.id ? (
              <Expand
                className="h-10 w-10 rotate-180 transition-all duration-200"
                title="Minimize"
                alt={`Minimize chapter ${card.id} content`}
              />
            ) : (
              <Expand
                className="h-10 w-10 transition-all duration-200"
                title="Expand"
                alt={`Expand chapter ${card.id} content`}
              />
            )}
          </button>
        </div>
